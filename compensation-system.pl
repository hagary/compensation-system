...
compensate(IN, OUT):-
  ...
  labeling([ffc, min(C)], [CompSlot, CompDay, CompWeek, CompRoom]),
  write(C).
...
member_times(1, [(D1,S1)|T],(D2,S2)):-
  D1 #= D2,
  S1 #= S2.

member_times(X, [(D1,S1)|T],(D,S)):-
  X #> 1,
  X1 #= X - 1,
  member_times(X1,T,(D,S)).
...
cost_of_time((StartW, StartD), Times, (CompWeek, CompD, CompSlot),C):-
  write('prefered cost '),
  StartW #= CompWeek,
  member_times(C,Times,(CompD, CompSlot)).
